...
def add_to_cart(menu_id: int, quantity: int = 1) -> str:
    """사용자가 요청한 메뉴를 장바구니에 추가합니다.
    menu_id: 메뉴의 고유 번호 (반드시 get_menu_info를 통해 확인된 ID여야 함)
    quantity: 수량 (따로 언급 없으면 1)
    """
    return f"[CART_ADD:{menu_id}:{quantity}]"
...
2. 사용자가 "이거 장바구니에 담아줘", "아메리카노 2잔 예약해줘" 등의 요청을 하면 반드시 'add_to_cart' 도구를 사용해몽!
   - 장바구니에 담기 전에도 반드시 'get_menu_info'로 메뉴의 진짜 ID를 확인해야 해몽! (._.)
3. 도구 실행 결과로 나오는 [NAV:target] 또는 [CART_ADD:ID:수량] 태그를 답변에 포함하면 프론트엔드가 알아서 처리해줄 거거몽!
...
def chat(messages: list[dict]) -> str:
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            ...
            tools=[get_menu_info, get_cafe_settings, navigate, add_to_cart]
        )
    )
